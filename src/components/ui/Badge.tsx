interface BadgeProps {
  ...
  variant?: 'primary' | 'secondary' | 'accent' | 'danger' | 'warning' | 'info' | 'gray' | 'purple'
    | 'green' | 'red' | 'yellow' | 'blue'; // legacy aliases kept for compatibility
  size?: 'sm' | 'md';
}
...
export const Badge = ({ children, variant = 'gray', size = 'sm' }: BadgeProps) => (
  <span className={`inline-flex items-center rounded-full font-medium ${size === 'sm' ? 'px-2 py-0.5 text-xs' : 'px-3 py-1 text-sm'} ${variants[variant] ?? variants.gray}`}>
    {children}
  </span>
);
