...
const scoreColor = (score: number) => {
  if (score >= 80) return 'text-agro-primary';
  if (score >= 60) return 'text-agro-secondary';
...
export const ESGRastreabilidade = () => {
  ...
  const ibamaCARStatus = (status: string) => {
    if (status === 'OK' || status === 'Conforme' || status === 'Registrado') return { icon: CheckCircle, color: 'text-agro-primary' };
    if (status === 'Pendente' || status === 'Em análise') return { icon: AlertCircle, color: 'text-agro-secondary' };
    return { icon: XCircle, color: 'text-agro-danger' };
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-text-primary">ESG e Rastreabilidade</h2>
          <p className="text-sm text-text-muted mt-0.5">Score Ambiental · Créditos de Carbono · Compliance</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 bg-gradient-to-br from-[#064E3B]/40 to-[#064E3B]/10 border border-[#047857]/40 rounded-[12px] p-6 flex flex-col items-center justify-center text-center">
          <div className="p-4 bg-[#064E3B]/50 rounded-[9999px] mb-4">
            <Leaf size={36} className="text-agro-primary" />
          </div>
          <div className="text-xs text-text-secondary font-medium uppercase tracking-wide mb-2">Score ESG Global</div>
          <div className="text-6xl font-bold text-[#6EE7B7] mb-1">{esgScore.total}</div>
          <div className="text-sm text-text-secondary mb-3">de 100 · Bom Desempenho</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {esgScore.categorias.map((cat) => (
          <Card key={cat.nome} className="text-center">
            ...
            <div className="text-xs text-text-secondary font-semibold mb-2">{cat.nome}</div>
            ...
            <div className="text-xs text-text-muted mt-1.5">Meta: {cat.meta}</div>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="px-5 pb-4">
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-surface rounded-[12px] p-3 text-center">
                <div className="text-xl font-bold text-agro-primary">{carbonoCreditos.reduce((a, b) => a + b.creditos, 0)}</div>
                <div className="text-xs text-text-muted">Créditos Totais</div>
              </div>
              <div className="bg-surface rounded-[12px] p-3 text-center">
                <div className="text-xl font-bold text-text-primary">
                  R$ {carbonoCreditos.reduce((a, b) => a + b.total, 0).toLocaleString('pt-BR')}
                </div>
                <div className="text-xs text-text-muted">Valor Portfólio</div>
              </div>
              <div className="bg-surface rounded-[12px] p-3 text-center">
                <div className="text-xl font-bold text-agro-secondary">
                  R$ {(carbonoCreditos.reduce((a, b) => a + b.total, 0) / carbonoCreditos.reduce((a, b) => a + b.creditos, 0)).toFixed(0)}
                </div>
                <div className="text-xs text-text-muted">Preço Médio/tCO₂</div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {carbonoCreditos.map((c, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                  <div>
                    <div className="text-xs font-semibold text-text-primary">{c.projeto}</div>
                    <div className="text-xs text-text-muted">{c.creditos} tCO₂ · R$ {c.precoUnitario}/t</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-agro-primary">R$ {c.total.toLocaleString('pt-BR')}</div>
        ...
        <Card padding={false}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left text-xs text-text-muted font-medium px-5 py-3">Área</th>
                  <th className="text-right text-xs text-text-muted font-medium px-3 py-3">Ha</th>
                  <th className="text-center text-xs text-text-muted font-medium px-3 py-3">CAR</th>
                  <th className="text-center text-xs text-text-muted font-medium px-3 py-3">IBAMA</th>
                  <th className="text-center text-xs text-text-muted font-medium px-3 py-3">SIF</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={i} className="border-b border-border/50 hover:bg-surface/40">
                      <td className="px-5 py-3">
                        <div className="text-xs font-semibold text-text-primary">{area.area}</div>
                        <div className="text-xs text-text-muted">{area.cultura}</div>
                      </td>
                      <td className="px-3 py-3 text-right text-xs font-mono text-text-secondary">{area.hectares}</td>
          ...
          <div className="p-5 pt-3 space-y-3">
            <div className="text-xs font-semibold text-text-muted uppercase tracking-wide">Selos e Certificações</div>
            <div className="flex flex-wrap gap-2">
              {['Soja Plus', 'RTRS', 'Rainforest Alliance', 'Non-GMO', 'Carbon Neutral 2025', 'BONSUCRO'].map(selo => (
                <div key={selo} className="flex items-center gap-1.5 bg-[#064E3B]/30 border border-[#047857]/40 rounded-[9999px] px-3 py-1">
                  <Award size={11} className="text-agro-primary" />
